...
- **Управление умными устройствами.** Интеллектуальное управление умными устройствами:
    * Контекст _Управление устройствами_: Регистрация новых устройств, их подключение и конфигурация, мониторинг статуса. Ключевые сущности - `Device`(устройство), `DeviceProfile`(спецификация устройства).
    * Контекст _Управление состоянием_: Чтение текущих показаний с датчиков (температура, свет включен/выключен) и отправка команд (включить отопление, включить свет) и т.п. Ключевые сущности - `DeviceState`(текущее состояние устройства), `Command` (команда конкретному устройству).
...
- **Безопасность и доступ.** Управление пользователями и правами доступа.
    * Контекст _Пользователи и дома_: Управление пользователями и правами доступа. Ключевые сущности - `User`(пользователь), `Home`(дом), `Location`(помещение).
